...
import { SignSession } from "./eddsa/signingsession";
import { SynchronizationStatus, SyncSession } from './eddsa/syncsession';

import { CompoundKeyEddsa } from 'crypto-core-async';

export abstract class EddsaCurrencyWallet extends CurrencyWallet {
  ...
  public async sync(options: any) {
    this.compoundKey = await CompoundKeyEddsa.fromOptions({
      curve: 'ed25519',
  ...
  public async syncDuplicate(other: CurrencyWallet) {
    const otherEddsa = other as EddsaCurrencyWallet;

    this.compoundKey = otherEddsa.compoundKey;
    this.publicKey = otherEddsa.publicKey;
  }
