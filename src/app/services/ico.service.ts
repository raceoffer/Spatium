...
export class ICOService {
  ...
  public async getCampaignList() {
    const result = await this.getIcoContract().methods.getCampaignsList().call();
    const campaings = [];
    ...
    for (let i = 0; i < result[0].length; i++) {
        const hash = Base58.encode(Buffer.concat([Buffer.from([18, 32]), Buffer.from(result[2][i].slice(2), 'hex')]));
  ...
  public async getCampaign(address: string) {
      const result = await this.getIcoContract().methods.getCampaign(address).call();
      const data = JSON.parse(this.web3.utils.hexToUtf8(result));
  ...
  public async addCampaign(сampaign: IcoCampaign) {
      const hash = Buffer.from(Base58.decode(сampaign.ipfsFolder).slice(2)).toString('hex');

      const addCampaignMethod = this.getIcoContract().methods.addCampaign(сampaign.address,
      ...
      const addCampaignAbi = addCampaignMethod.encodeABI();
      const addCampaignGas = await addCampaignMethod.estimateGas();
  ...
  public async removeCampaign(address: string) {
    const removeCampaignMethod = this.getIcoContract().methods.removeCampaign(address);
    const removeCampaignAbi = removeCampaignMethod.encodeABI();
    const removeCampaignGas = await removeCampaignMethod.estimateGas();
    const account = this.web3.eth.accounts.privateKeyToAccount(this.getSecret());
    const tx = {
        from: account.address,
        to: this.contractAddress,
