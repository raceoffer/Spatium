cordova.define("cordova-plugin-bluetooth.BluetoothProxy", function(require, exports, module) {
    ...
    function onAdded(devinfo) {
        if (_discoveredCallback) {
            _discoveredCallback({name: devinfo.name, address: devinfo.id}, { keepCallback: true })
        }
    ...
    cordova.commandProxy.add("Bluetooth", {
                ...
                null).done(function (services) {
                    ...
                    if (services.length > 0) {
                        for (var i = 0; i < services.length; i++) {
                            var service = { name: services[i].name, address: services[i].id }
        ...
        connect: function(successCallback,errorCallback,params) {
            ...
            var device = params[0]
            rfcomm.RfcommDeviceService.fromIdAsync(device.address).done(
                function(service) {
                    if (service === null) {
                        var errorMessage = "Access to the device is denied because the application was not granted access"
